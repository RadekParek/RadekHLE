...
Set `TOUCHHLE_TRACE_TRANSLATOR=1` or `TOUCHHLE_GLES1_GLES2_LOG=1` to enable structured logs for matrix operations, projection parameters, viewport changes, texture coordinates, vertex samples, clip-space transformations and NDC values. The built-in `aggressive` rotation mode is enabled by default; it applies two clockwise quarter-turns (180° total) while preserving Z, scale and perspective terms. Override it with `TOUCHHLE_GLES1_ROTATION_FIX=none` or `diagnostic` for diagnosis, `clockwise` or `counter-clockwise` for a single quarter-turn. The seven stages are logged for every MVP upload.

`TOUCHHLE_GLES1_ROTATION_FIX=aggressive` enables the default compatibility transform: two clockwise 90° screen rotations (180° total). The axis-swap, Y-inversion, and horizontal-flip candidates remain disabled to avoid competing coordinate conversions. For a graphics-only adjustment that does not change the emulated device orientation, use `--render-rotation=default|-90|-180|90|180`.
