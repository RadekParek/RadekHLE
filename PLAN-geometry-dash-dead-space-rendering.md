...
- The supplied Geometry Dash log is from MetalHLE 9.1 (`4d3b8b8`), not the current 9.2 source. It shows a live render loop, a 480x320 landscape surface, and an early `TOUCHHLE_PRESENT_STRETCH_TO_VIEWPORT=1` override; the reported FPS later recovers to roughly 60–75. MetalHLE 0.1 no longer applies that stretch override to Geometry Dash, so the old log cannot verify the current path.
